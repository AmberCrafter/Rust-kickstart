...
impl Solution {
   pub fn solve(console: &mut Console) -> (String,String) {
        let name: String = console.read();
        let winner = match name.chars().last().unwrap() {
            'A' | 'E' | 'I' | 'O' | 'U' => "Alice".to_string(),
            'a' | 'e' | 'i' | 'o' | 'u' => "Alice".to_string(),
            'Y' | 'y' => "nobody".to_string(),
            _ => "Bob".to_string()
        };
        (name, winner)
   }
...
fn main() {
   ...
   for i in 0..nums  {
       ...
       console.write(format!(
           "Case #{idx}: {name} is ruled by {value}.\n",
           idx=i+1,
           name=result.0,
           value=result.1
       ))
